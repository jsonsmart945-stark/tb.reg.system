...
const SMTP_CONFIG = {
  ...
  auth: {
    ...
    pass: 'jhxd iwnu ggqs wnnm'   // Gmail App Password
  }
};
...
// Serve the registration HTML directly
app.use(express.static(path.join(__dirname)));
...
transporter.verify((err) => {
  if (err) {
    ...
    console.log('✅ Email server ready — will send to:', SCHOOL_EMAIL);
  }
});
...
app.post('/api/test-email', async (req, res) => {
  try {
    const info = await transporter.sendMail({
      ...
      html: '<p>Barua pepe ya majaribio kutoka mfumo wa usajili wa <strong>Tabora Boys Secondary School</strong>. Mfumo unafanya kazi vizuri!</p>'
    });
...
app.post('/api/register', upload.single('pdf'), async (req, res) => {
  try {
    ...
    const submissionDate = new Date(d.submissionDate).toLocaleString('en-GB', { timeZone: 'Africa/Dar_es_Salaam' });
    ...
    const section = (icon, title, rows) => `
      <div style="margin-bottom:20px;">
        <div style="background:#1a365d;color:white;padding:8px 14px;font-size:13px;font-weight:700;border-radius:6px 6px 0 0;display:flex;align-items:center;gap:8px;">
          ${icon} ${title}
        </div>
...
<div style="max-width:650px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.1);margin-top:24px;margin-bottom:24px;">

  <!-- Header -->
  <div style="background:linear-gradient(135deg,#0d1f3c,#1a365d);padding:28px 32px;display:flex;align-items:center;gap:20px;">
    <div style="width:60px;height:60px;background:#c8962a;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.8rem;flex-shrink:0;">🏫</div>
    <div>
      <div style="color:white;font-size:17px;font-weight:800;letter-spacing:0.5px;">SHULE YA SEKONDARI TABORA WAVULANA</div>
      <div style="color:#f0c84a;font-size:11px;margin-top:3px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Usajili Mpya — Kidato cha Kwanza 2026</div>
    </div>
  </div>
  <div style="height:4px;background:linear-gradient(90deg,#c8962a,#f0c84a,#c8962a);"></div>

  <!-- Alert banner -->
  <div style="padding:16px 28px;background:#e8f5e9;border-left:4px solid #2d6a4f;display:flex;align-items:center;gap:12px;">
    <span style="font-size:1.6rem;">✅</span>
    <div>
      <div style="font-weight:700;color:#1b5e20;font-size:14px;">Mwanafunzi Mpya Amejisajili</div>
      <div style="color:#2e7d32;font-size:12px;margin-top:2px;">Fomu imewasilishwa tarehe ${submissionDate} (EAT)</div>
    </div>
  </div>
  ...
  <div style="padding:24px 28px;">
    ...
    ${section('🏥', 'UCHUNGUZI WA AFYA',
      ...
      row('Hali ya Ujumla ya Afya', `<strong style="color:#2d6a4f;">${d.generalHealth}</strong>`) +
    ...
    <div style="margin-top:20px;padding:14px 18px;background:#fffbf0;border:1px solid #ffe082;border-radius:8px;font-size:12px;color:#7a5400;line-height:1.7;">
      <strong>📎 Kiambatisho:</strong> PDF iliyo kamili ya fomu ya usajili imeambatishwa kwenye barua pepe hii. Hati hii ina kurasa 4 za kumbukumbu yote ya mwanafunzi.
    </div>
  </div>
  ...
  <div style="background:#f7f9fc;padding:16px 28px;border-top:1px solid #eef2f7;text-align:center;font-size:11px;color:#888;line-height:1.8;">
    ...
    // Delete temp file after 10s
    if (pdfPath) {
      setTimeout(() => fs.unlink(pdfPath, () => {}), 10000);
    }

    console.log(`✅ Registered: ${fullName} — email sent [${info.messageId}]`);
    res.json({ success: true, message: 'Usajili umefanikiwa na barua pepe imetumwa', emailSent: true, messageId: info.messageId });
...
║                                                              ║
║  Open tabora_kujiunga_2026.html in the browser, or visit:   ║
║  http://localhost:${PORT}/tabora_kujiunga_2026.html            ║
╚══════════════════════════════════════════════════════════════╝
  `);
});
